    ...
    public class NxrLaserPointer : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (bHit && hit.distance < zDistance)
            {
                ...
                if (losdot != null)
                {
                    bool isUICanvasObject = (hit.transform.GetComponent<Canvas>() != null) && (hit.transform.GetComponent<NxrUICanvas>());
                    if (isUICanvasObject)
                    {
                        dist = zDistance;
                        losdot.SetActive(false); 
                        ...
                        if (eventSystem)
                        {
                            ...
                            // Debug.Log("RaycastResultsCount:"+raycastResults.Count);
                            for (var i = raycastResults.Count-1; i >=0; i--)
                            {
                                // Debug.Log("RaycastResultName"+i+":"+raycastResults[i].gameObject.name);
